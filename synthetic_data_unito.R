...
for (i in 2:n){
  ...
  if (u <= beta){
    ...
    if (v > ((theta + (dummy-m1_mean)*sigma) / (theta+n-m1_mean)) && v < ((1-sigma)/(theta+n-m1_mean))){
      t <- round(runif(1, 1, i),digits=0) # genera un indice tra quelli disponibili
      while (J[t]==0 || length(which(X == X[t]))>1){
        t <- round(runif(1, 1, i),digits=0)
        # rifalla finché non ho un valore non contaminante con frequenza 1
      }
